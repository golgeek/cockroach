...
func TestProcessTasksMaintenanceRoutine(t *testing.T) {
	...
	mockRepo.On(
		"PurgeTasks", mock.Anything, mock.Anything, taskServiceOpts.PurgeDoneTaskOlderThan, tasks.TaskStateDone,
	).Return(1, nil)
	mockRepo.On(
		"PurgeTasks", mock.Anything, mock.Anything, taskServiceOpts.PurgeFailedTaskOlderThan, tasks.TaskStateFailed,
	).Return(0, nil)

	errChan := make(chan error)
	err := taskService.processTasksMaintenanceRoutine(ctx, logger.DefaultLogger, errChan)
	assert.Nil(t, err)
	select {
...
func TestProcessTask_Error(t *testing.T) {
	...
	taskService.RegisterTasksService(service)
	err := taskService._processTask(context.Background(), logger.DefaultLogger, task)
...
func TestProcessTask_Unmanaged(t *testing.T) {
	...
	err := taskService._processTask(context.Background(), logger.DefaultLogger, task)
	assert.ErrorIs(t, err, types.ErrTaskTypeNotManaged)
}
