...
import (
	...
	tasksrepomock "github.com/cockroachdb/cockroach/pkg/cmd/roachprod-centralized/repositories/tasks/mock"
...
func TestGetTasks_Error(t *testing.T) {
	...
	mockRepo.On("GetTasks", mock.Anything).Return(nil, expectedError)
...
func TestCreateTaskIfNotAlreadyPlanned_ErrorGet(t *testing.T) {
	...
	mockRepo.On("GetTasks", mock.Anything).Return(
...
func TestCreateTaskIfNotAlreadyPlanned_CreatesNew(t *testing.T) {
	...
	mockRepo.On("GetTasks", mock.Anything).Return([]tasks.ITask{}, nil)
	fakeTask := &MockTask{
		Task: tasks.Task{
			ID:   uuid.MakeV4(),
			Type: "fake_type",
...
func TestCreateTaskIfNotAlreadyPlanned_CreatesNew_FailedExists(t *testing.T) {
	...
	mockRepo.On("GetTasks", mock.Anything).Return(tasksList, nil)
...
func TestCreateTaskIfNotAlreadyPlanned_ReturnsExisting(t *testing.T) {
	...
	mockRepo.On("GetTasks", mock.Anything).Return([]tasks.ITask{
		pendingTask,
	}, nil).Once()
