...
// Init initializes the tasks service; it starts the task processing routine.
...
// TODO (ludo): Implement filtering by type and state.
...
func (s *Service) GetTask(
	...
	if err != nil {
		if errors.Is(err, tasksrepo.ErrTaskNotFound) {
			return nil, utils.NewPublicError(ErrTaskNotFound)
		}
		return nil, err
	}
...
func (s *Service) CreateTaskIfNotAlreadyPlanned(
	...
	storedTasks, err := s.store.GetTasks(ctx)
	if err != nil {
		return nil, err
	}
