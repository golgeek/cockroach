...
type InputCreateClusterDTO struct {
	OperationBeginTime time.Time `json:"operation_begin_time" binding:"required,datetime"`
	OperationEndTime   time.Time `json:"operation_end_time" binding:"required,datetime"`
	cloud.Cluster
}
...
func (dto *ClustersResultError) GetAssociatedStatusCode() int {
	...
	case errors.As(err, &clusters.ErrClusterNotFound):
		return http.StatusNotFound
	case errors.As(err, &clusters.ErrClusterAlreadyExists):
