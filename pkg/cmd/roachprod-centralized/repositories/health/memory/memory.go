...
func (r *MemHealthRepo) CleanupDeadInstances(
	...
	for instanceID, instance := range r.instances {
		...
		if isDead && beyondRetention && len(toDelete) < batchSize {
...
// In-memory implementation: always return false (no cross-instance coordination).
// Each instance will create its own cleanup task, but tasks system will handle deduplication.
func (r *MemHealthRepo) HasRecentCleanupTask(
