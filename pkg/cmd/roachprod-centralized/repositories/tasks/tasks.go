...
type ITasksRepository interface {
	GetTasks(context.Context) ([]tasks.ITask, error)
	...
	PurgeTasks(context.Context, time.Duration) error
	GetTasksForProcessing(context.Context, *slog.Logger, chan<- tasks.ITask, uuid.UUID) error
}
