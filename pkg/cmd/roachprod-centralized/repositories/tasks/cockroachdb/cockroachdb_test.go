...
func setupTestDB(_ *testing.T) *sql.DB {
...
func TestCRDBTasksRepo_ClaimNextTask(t *testing.T) {
	...
	claimedTask, found, err := repo.claimNextTask(ctx, logger.DefaultLogger, consumerID)
	require.NoError(t, err)
	...
	_, found, err = repo.claimNextTask(ctx, logger.DefaultLogger, consumerID)
	require.NoError(t, err)
	require.False(t, found)
}

func TestCRDBTasksRepo_PollingInterval(t *testing.T) {
	repo := NewTasksRepository(nil, Options{
		BasePollingInterval: 100 * time.Millisecond,
		MaxPollingInterval:  2 * time.Second,
	})

	// Initial interval should be base
	require.Equal(t, 100*time.Millisecond, repo.getCurrentInterval())

	// No tasks found should increase interval
	repo.updatePollingInterval(false)
	interval1 := repo.getCurrentInterval()
	require.True(t, interval1 > 100*time.Millisecond)

	// More misses should increase further
	repo.updatePollingInterval(false)
	interval2 := repo.getCurrentInterval()
	require.True(t, interval2 > interval1)

	// Finding tasks should reset to base
	repo.updatePollingInterval(true)
	require.Equal(t, 100*time.Millisecond, repo.getCurrentInterval())
}
