...
func (c *Client) CreateCluster(ctx context.Context, l *logger.Logger, cluster *cloudcluster.Cluster) error {
	// If the centralized API is disabled, return an error.
	if !c.config.Enabled {
		return ErrDisabled
	}

	url := fmt.Sprintf("%s%s", c.config.BaseURL, clustertypes.ControllerPath)
	payload := clustertypes.InputCreateClusterDTO{Cluster: *cluster}

	return c.makeRequest(ctx, l, "POST", url, payload, nil, "create cluster")
}

func (c *Client) UpdateCluster(ctx context.Context, l *logger.Logger, cluster *cloudcluster.Cluster) error {
	// If the centralized API is disabled, return an error.
	if !c.config.Enabled {
		return ErrDisabled
	}

	url := fmt.Sprintf("%s%s/%s", c.config.BaseURL, clustertypes.ControllerPath, cluster.Name)
	payload := clustertypes.InputUpdateClusterDTO{Cluster: *cluster}

	return c.makeRequest(ctx, l, "PUT", url, payload, nil, "update cluster")
}

// DeleteCluster notifies the centralized API about a cluster deletion.
func (c *Client) DeleteCluster(ctx context.Context, l *logger.Logger, clusterName string) error {
	...
	url := fmt.Sprintf("%s%s/%s", c.config.BaseURL, clustertypes.ControllerPath, clusterName)

	return c.makeRequest(ctx, l, "DELETE", url, nil, nil, "delete cluster")
}
...
func (c *Client) makeRequest(
	...
	for {
		...
		c.logDebug(l, "centralized API %s failed (attempt %d/%d): %v", operation, attempt, c.config.RetryAttempts, err)
	...
	c.logError(l, "centralized API %s failed after %d attempts: %v", operation, attempt, lastErr)
...
// Logging helpers that respect the logger interface
func (c *Client) logInfo(l *logger.Logger, format string, args ...interface{}) {
	if l != nil {
		l.Printf("INFO: "+format, args...)
	}
}

func (c *Client) logDebug(l *logger.Logger, format string, args ...interface{}) {
	if l != nil {
		l.Printf("DEBUG: "+format, args...)
	}
}

func (c *Client) logError(l *logger.Logger, format string, args ...interface{}) {
	if l != nil {
...
// isNotFoundError checks if the error indicates a 404 Not Found.
