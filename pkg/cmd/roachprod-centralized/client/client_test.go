...
func TestClient_DeleteCluster_Success(t *testing.T) {
	// Mock server
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method == "DELETE" && r.URL.Path == "/clusters/test-cluster" {
			w.WriteHeader(http.StatusOK)
		} else {
	...
	err = client.DeleteCluster(context.Background(), createTestLogger(t), "test-cluster")
...
func TestClient_RetryOnServerError(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if callCount == 1 {
			// First GET call fails with server error
			w.WriteHeader(http.StatusInternalServerError)
			_, _ = w.Write([]byte("server error"))
		} else if callCount == 2 {
			// PUT call succeeds - update cluster
			w.WriteHeader(http.StatusOK)
			_ = json.NewEncoder(w).Encode(map[string]interface{}{
				"data": map[string]interface{}{
					"name": "test-cluster",
