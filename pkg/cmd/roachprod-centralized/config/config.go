...
func InitConfigWithFlagSet(flagSet *pflag.FlagSet) (*types.Config, error) {
	...
	if configFile != "" {
		...
		e.SetConfigType(strings.TrimPrefix(filepath.Ext(configFile), "."))
		if err := e.ReadConfig(file); err != nil {
	...
	err = processing.BindNonSliceFlagsToViper(flagSet, e, config)
	if err != nil {
