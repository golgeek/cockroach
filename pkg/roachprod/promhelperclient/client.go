...
var promRegistrationUrl = config.EnvOrDefaultString(
...
func NewPromClient(options ...Option) (*PromClient, error) {
	//func NewPromClient(iapTokenSource *roachprodutil.IAPTokenSource) (*PromClient, error) {

	c := &PromClient{
		promUrl:  promRegistrationUrl,
		disabled: promRegistrationUrl == "",
		supportedPromProviders: map[string]Reachability{
			gce.ProviderName:   Public,
